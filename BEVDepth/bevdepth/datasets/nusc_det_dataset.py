...
class NuscDetDataset(Dataset):
    ...
    def get_image(self, cam_infos, cams, lidar_infos=None):
        ...
        ##################TODO: jinlong
        imgname =os.path.join(cam_infos[0][cams[1]]['filename'])
        ##########################
        for cam in cams:
        # cam_f = [cams[1]]
        # for cam in cam_f:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                img = torch.from_numpy(img).permute(2, 0, 1)
                # img = tv_resize(img, (512, 512))
                # img = tv_resize(img, (396, 704))
                # img = tv_crop(img, 140, 0, 256, 704)
                # img = img
                imgs.append(img)
    ...
    def __getitem__(self, idx):
        ...
        #####TODO:jinlong
        # ret_list.append(image_data_list[-1])
        return ret_list
